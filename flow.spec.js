...
test('landing CTAs start the local onboarding flow', async ({ page }) => {
  ...
  await expect(page.getByText('lihi 帶你一次完成。')).toBeVisible();
  await expect(page.getByText('註冊即送 200 點 簡訊點數')).toHaveCount(0);
...
test('email registration validates and advances', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '建立你的 lihi 帳號' })).toBeVisible();
...
test('KYC upload flow advances', async ({ page }) => {
  ...
  await expect(page.getByText(/為配合 NCC《電信管理法》/)).toBeVisible();
...
test('domain choice and application flow complete', async ({ page }) => {
  ...
  await expect(page.getByText('14 個工作天')).toBeVisible();
  await expect(page.locator('.success-reading li')).toHaveCount(4);
});
